...
final class Elev8_OS_Production_Workspace_Module {
    ...
    public static function shortcode(): string {
        ...
        $selected_context_thread = absint($_GET['elev8_context_conversation'] ?? 0);
        $open_conversation_job = $selected_context_thread ? absint(get_post_meta($selected_context_thread, '_elev8_conversation_context_id', true)) : 0;
        $conversation_return_url = self::url(array_filter([
            'production_status' => $filters['status'],
            'production_priority' => $filters['priority'],
            'production_owner' => $filters['assigned_user_id'],
            'production_search' => $filters['search'],
            'production_overdue' => $filters['overdue'] ? 1 : 0,
            'production_type' => $filters['job_type'],
            'production_source' => $filters['source'],
            ...
            <section style="background:#fff;border:1px solid #ddd;border-radius:18px;padding:18px;overflow:auto"><h2 style="margin-top:0"><?php esc_html_e('Production Queue','elev8-os'); ?></h2>
                ...
                <?php foreach ($data['queue'] as $item): ?><tr>
                    ...
                    <td>
                        ...
                        <?php if (class_exists('Elev8_OS_Embedded_Conversation_Service')): $job_unread = Elev8_OS_Embedded_Conversation_Service::unread_count('production_job', (int)$item['id']); ?><details class="elev8-production-conversation" <?php echo $open_conversation_job === (int)$item['id'] ? 'open' : ''; ?>><summary><?php esc_html_e('Conversation','elev8-os'); ?><?php if ($job_unread > 0): ?> <span style="display:inline-flex;min-width:20px;height:20px;align-items:center;justify-content:center;border-radius:999px;background:#6d28d9;color:#fff;font-size:11px"><?php echo esc_html((string)$job_unread); ?></span><?php endif; ?></summary><?php echo Elev8_OS_Embedded_Conversation_Service::render('production_job', (int)$item['id'], ['title' => sprintf(__('Production: %s','elev8-os'), $item['title']), 'return_url' => $conversation_return_url, 'participant_user_ids' => [(int)$item['assigned_user_id']]]); ?></details><?php endif; ?>
