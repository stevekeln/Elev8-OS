...
final class Elev8_OS_Observation_Registry_Module {
    ...
    private static function recommendation_card(array $item): void {
        echo '<article style="background:#fff;border:1px solid #ddd;border-radius:14px;padding:18px;margin:0 0 14px"><div style="display:flex;justify-content:space-between;gap:20px"><div><div style="font-size:12px;text-transform:uppercase">'.esc_html($item['classification'].' · '.$item['severity']).'</div><h3 style="margin:6px 0">'.esc_html($item['title']).'</h3><p>'.esc_html($item['summary']).'</p><p><strong>'.esc_html__('Suggested action:','elev8-os').'</strong> '.esc_html($item['suggested_action']).'</p><p><strong>'.esc_html__('Expected benefit:','elev8-os').'</strong> '.esc_html($item['expected_benefit']).'</p><small>'.sprintf(esc_html__('%1$d supporting observations · %2$d%% calibrated confidence · Suggested owner: %3$s','elev8-os'),count((array)$item['observation_ids']),(int)$item['confidence'],$item['suggested_owner_name']).'</small>';
        $calibration=is_array($item['confidence_calibration']??null)?$item['confidence_calibration']:[];
        if($calibration){echo '<div style="margin-top:10px;padding:10px;border-left:3px solid #555;background:#f8f8f8"><strong>'.esc_html__('Confidence calibration','elev8-os').'</strong><p style="margin:4px 0 0">'.esc_html((string)($calibration['explanation']??'')).'</p></div>';}
        echo '</div><strong>'.esc_html(ucfirst($item['status'])).'</strong></div>';
