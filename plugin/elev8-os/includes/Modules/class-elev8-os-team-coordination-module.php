...
final class Elev8_OS_Team_Coordination_Module {
    ...
    public static function init(): void {
        ...
        add_action('admin_post_elev8_save_availability_exception', [__CLASS__, 'save_availability_exception']);
        add_action('admin_post_elev8_delete_availability_exception', [__CLASS__, 'delete_availability_exception']);
        add_action('admin_post_elev8_save_coordination_credential', [__CLASS__, 'save_coordination_credential']);
        add_action('admin_post_elev8_delete_coordination_credential', [__CLASS__, 'delete_coordination_credential']);
        add_action('admin_post_elev8_save_work_credential_requirements', [__CLASS__, 'save_work_credential_requirements']);
    ...
    private static function render(): string {
        ...
        foreach ($users as $assignable_user) {
            $coordination_profiles[(int) $assignable_user->ID] = [
                ...
                'availability_exceptions' => Elev8_OS_Team_Coordination_Evidence_Service::exceptions((int) $assignable_user->ID),
                'credentials' => Elev8_OS_Team_Coordination_Evidence_Service::credentials((int) $assignable_user->ID),
            ];
        }
        ...
        <main class="elev8-team-coordination" style="max-width:1200px;margin:0 auto;padding:24px">
            ...
            <section style="background:#fff;border:1px solid #ddd;border-radius:18px;padding:20px;margin-bottom:18px"><h2 style="margin-top:0"><?php esc_html_e('Availability, exceptions, skills, and credential evidence', 'elev8-os'); ?></h2><p><?php esc_html_e('This evidence improves handoff suggestions only. It is not attendance, leave approval, employment scheduling, professional certification, or a permission grant. Never enter passwords, secret keys, or credential numbers.', 'elev8-os'); ?></p>
                ...
                <?php foreach ($users as $assignable): $profile=$coordination_profiles[(int)$assignable->ID]; $can_edit=((int)$assignable->ID===(int)$user->ID)||Elev8_OS_Team_Coordination_Service::can_coordinate($user); $can_verify=Elev8_OS_Team_Coordination_Service::can_coordinate($user); ?>
                    ...
                    <?php if ($can_verify && $profile['skills']): ?><details><summary><?php esc_html_e('Verify declared skills','elev8-os'); ?></summary><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><input type="hidden" name="action" value="elev8_verify_coordination_skills"><input type="hidden" name="user_id" value="<?php echo esc_attr((string)$assignable->ID); ?>"><?php wp_nonce_field('elev8_verify_coordination_skills_'.$assignable->ID); ?><?php foreach ($profile['skills'] as $skill): ?><label style="display:block"><input type="checkbox" name="verified_skills[]" value="<?php echo esc_attr($skill); ?>" <?php checked(in_array($skill,$profile['verified_skills'],true)); ?>> <?php echo esc_html($skill); ?></label><?php endforeach; ?><label><?php esc_html_e('Verification expires on (optional)','elev8-os'); ?><input type="date" name="verification_expires_on" style="display:block"></label><label><?php esc_html_e('Verification note','elev8-os'); ?><textarea name="verification_note" rows="2" style="display:block;width:100%"></textarea></label><p><button class="button" type="submit"><?php esc_html_e('Save verification evidence','elev8-os'); ?></button></p></form></details><?php endif; ?>
                    <?php if ($can_edit): ?><details><summary><?php esc_html_e('Date-specific availability exceptions','elev8-os'); ?></summary><?php foreach ($profile['availability_exceptions'] as $exception): ?><p><strong><?php echo esc_html($exception['date'].' — '.ucfirst($exception['state'])); ?></strong><?php if ($exception['note']): ?> · <?php echo esc_html($exception['note']); ?><?php endif; ?> <a href="<?php echo esc_url(wp_nonce_url(add_query_arg(['action'=>'elev8_delete_availability_exception','user_id'=>$assignable->ID,'exception_date'=>$exception['date']],admin_url('admin-post.php')),'elev8_delete_availability_exception_'.$assignable->ID.'_'.$exception['date'])); ?>"><?php esc_html_e('Remove','elev8-os'); ?></a></p><?php endforeach; ?><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><input type="hidden" name="action" value="elev8_save_availability_exception"><input type="hidden" name="user_id" value="<?php echo esc_attr((string)$assignable->ID); ?>"><?php wp_nonce_field('elev8_save_availability_exception_'.$assignable->ID); ?><label><?php esc_html_e('Date','elev8-os'); ?><input type="date" name="exception_date" required></label> <label><?php esc_html_e('State','elev8-os'); ?><select name="exception_state"><option value="unavailable"><?php esc_html_e('Unavailable','elev8-os'); ?></option><option value="limited"><?php esc_html_e('Limited','elev8-os'); ?></option><option value="available"><?php esc_html_e('Available override','elev8-os'); ?></option></select></label><label style="display:block"><?php esc_html_e('Note','elev8-os'); ?><textarea name="exception_note" rows="2" style="width:100%"></textarea></label><p><button class="button" type="submit"><?php esc_html_e('Save exception','elev8-os'); ?></button></p></form></details><?php endif; ?>
                    <?php if ($can_verify): ?><details><summary><?php esc_html_e('Credential and training evidence','elev8-os'); ?></summary><?php foreach ($profile['credentials'] as $credential): ?><p><strong><?php echo esc_html($credential['title']); ?></strong> · <?php echo esc_html(ucfirst($credential['status'])); ?><?php if ($credential['expires_on']): ?> · <?php echo esc_html(sprintf(__('expires %s','elev8-os'),$credential['expires_on'])); ?><?php endif; ?><?php if ($credential['reference']): ?><br><span style="color:#666"><?php echo esc_html($credential['reference']); ?></span><?php endif; ?> <a href="<?php echo esc_url(wp_nonce_url(add_query_arg(['action'=>'elev8_delete_coordination_credential','user_id'=>$assignable->ID,'credential_id'=>$credential['id']],admin_url('admin-post.php')),'elev8_delete_coordination_credential_'.$assignable->ID.'_'.$credential['id'])); ?>"><?php esc_html_e('Remove','elev8-os'); ?></a></p><?php endforeach; ?><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><input type="hidden" name="action" value="elev8_save_coordination_credential"><input type="hidden" name="user_id" value="<?php echo esc_attr((string)$assignable->ID); ?>"><?php wp_nonce_field('elev8_save_coordination_credential_'.$assignable->ID); ?><label><?php esc_html_e('Credential or training title','elev8-os'); ?><input type="text" name="credential_title" required style="display:block;width:100%"></label><label><?php esc_html_e('Related skill (optional)','elev8-os'); ?><input type="text" name="credential_skill" style="display:block;width:100%"></label><label><?php esc_html_e('Issuer (optional)','elev8-os'); ?><input type="text" name="credential_issuer" style="display:block;width:100%"></label><label><?php esc_html_e('Expires on (optional)','elev8-os'); ?><input type="date" name="credential_expires_on"></label><label><?php esc_html_e('Renewal reminder days','elev8-os'); ?><input type="number" min="1" max="365" name="credential_renewal_days" value="30"></label><label style="display:block"><?php esc_html_e('Secure reference location or internal note','elev8-os'); ?><input type="text" name="credential_reference" style="display:block;width:100%" placeholder="Secure Drive folder or internal record name"></label><label><?php esc_html_e('WordPress Media attachment ID (optional)','elev8-os'); ?><input type="number" min="0" name="credential_attachment_id"></label><p style="color:#666"><?php esc_html_e('Store only a safe reference. Never enter passwords, secret keys, license keys, access codes, or full credential numbers.','elev8-os'); ?></p><p><button class="button" type="submit"><?php esc_html_e('Save credential evidence','elev8-os'); ?></button></p></form></details><?php endif; ?></article>
                <?php endforeach; ?></div>
            </section>
            ...
            <?php foreach ($snapshot['bottlenecks'] as $item): ?><article style="border-top:1px solid #eee;padding:14px 0"><h3 style="margin:0"><?php echo esc_html($item['title']); ?></h3><p><?php echo esc_html(sprintf(__('Owner: %1$s · Bottleneck score: %2$d · Waiting on: %3$d · Blocking: %4$d', 'elev8-os'), $item['owner_name'], $item['bottleneck_score'], count($item['open_dependencies']), count($item['dependent_ids']))); ?></p>
                <?php if (Elev8_OS_Team_Coordination_Service::can_change_work((int)$item['id'],$user)): ?><details><summary><?php esc_html_e('Manage dependencies or request handoff', 'elev8-os'); ?></summary>
                    ...
                    <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="margin:12px 0"><input type="hidden" name="action" value="elev8_save_work_credential_requirements"><input type="hidden" name="work_id" value="<?php echo esc_attr((string)$item['id']); ?>"><?php wp_nonce_field('elev8_save_work_credential_requirements_'.$item['id']); ?><label><?php esc_html_e('Credential or training evidence required','elev8-os'); ?><textarea name="credential_requirements" rows="2" style="display:block;width:100%" placeholder="safety training, networking, equipment authorization"><?php echo esc_textarea(implode(', ',Elev8_OS_Credential_Workflow_Service::requirements((int)$item['id']))); ?></textarea></label><?php $readiness=Elev8_OS_Credential_Workflow_Service::readiness((int)$item['id'],(int)($item['owner_user_id']??0)); if ($readiness['required']): ?><p style="color:#666"><?php echo esc_html($readiness['explanation']); ?><?php if ($readiness['missing']): ?><br><?php echo esc_html(sprintf(__('Missing: %s','elev8-os'),implode(', ',$readiness['missing']))); ?><?php endif; ?><?php if ($readiness['expired']): ?><br><?php echo esc_html(sprintf(__('Expired: %s','elev8-os'),implode(', ',$readiness['expired']))); ?><?php endif; ?></p><?php endif; ?><p><button class="button" type="submit"><?php esc_html_e('Save credential requirements','elev8-os'); ?></button></p></form>
    ...
    public static function verify_coordination_skills(): void {
        ...
        self::redirect(Elev8_OS_Team_Availability_Calendar_Service::verify_skills($user_id,(array)($_POST['verified_skills']??[]),sanitize_textarea_field(wp_unslash((string)($_POST['verification_note']??''))),sanitize_text_field(wp_unslash((string)($_POST['verification_expires_on']??'')))));
    }


    public static function save_availability_exception(): void {
        $user_id=absint($_POST['user_id']??0); check_admin_referer('elev8_save_availability_exception_'.$user_id);
        self::redirect(Elev8_OS_Team_Coordination_Evidence_Service::save_exception($user_id,sanitize_text_field(wp_unslash((string)($_POST['exception_date']??''))),sanitize_key((string)($_POST['exception_state']??'unavailable')),sanitize_textarea_field(wp_unslash((string)($_POST['exception_note']??'')))));
    }

    public static function delete_availability_exception(): void {
        $user_id=absint($_GET['user_id']??0); $date=sanitize_text_field(wp_unslash((string)($_GET['exception_date']??''))); check_admin_referer('elev8_delete_availability_exception_'.$user_id.'_'.$date);
        self::redirect(Elev8_OS_Team_Coordination_Evidence_Service::delete_exception($user_id,$date));
    }

    public static function save_coordination_credential(): void {
        $user_id=absint($_POST['user_id']??0); check_admin_referer('elev8_save_coordination_credential_'.$user_id);
        self::redirect(Elev8_OS_Team_Coordination_Evidence_Service::save_credential($user_id,[
            'title'=>wp_unslash((string)($_POST['credential_title']??'')), 'skill'=>wp_unslash((string)($_POST['credential_skill']??'')),
            'issuer'=>wp_unslash((string)($_POST['credential_issuer']??'')), 'expires_on'=>wp_unslash((string)($_POST['credential_expires_on']??'')),
            'renewal_days'=>absint($_POST['credential_renewal_days']??30), 'reference'=>wp_unslash((string)($_POST['credential_reference']??'')),
            'attachment_id'=>absint($_POST['credential_attachment_id']??0),
        ]));
    }

    public static function delete_coordination_credential(): void {
        $user_id=absint($_GET['user_id']??0); $credential_id=sanitize_key((string)($_GET['credential_id']??'')); check_admin_referer('elev8_delete_coordination_credential_'.$user_id.'_'.$credential_id);
        self::redirect(Elev8_OS_Team_Coordination_Evidence_Service::delete_credential($user_id,$credential_id));
    }


    public static function save_work_credential_requirements(): void {
        $work_id=absint($_POST['work_id']??0); check_admin_referer('elev8_save_work_credential_requirements_'.$work_id);
        self::redirect(Elev8_OS_Credential_Workflow_Service::save_requirements($work_id,[(string)wp_unslash($_POST['credential_requirements']??'')]));
    }
