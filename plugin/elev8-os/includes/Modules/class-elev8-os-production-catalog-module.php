...
final class Elev8_OS_Production_Catalog_Module {
    ...
    public static function init(): void {
        ...
        add_action('wp_ajax_elev8_os_quick_add_production_material',[__CLASS__,'ajax_quick_add_material']);
    ...
    public static function assets(string $hook): void {
        ...
        wp_enqueue_script('elev8-production-catalog',ELEV8_OS_URL.'assets/js/production-catalog.js',[],ELEV8_OS_VERSION,true);
        wp_localize_script('elev8-production-catalog','Elev8ProductionCatalog',[
            'ajaxUrl'=>admin_url('admin-ajax.php'),
            'materialNonce'=>wp_create_nonce('elev8_quick_add_production_material'),
        ]);
    }
                            ...
                            <article class="wizard-item">
                                ...
                                <div class="wizard-item__fields">
                                    ...
                                    <div class="financial-strip">
                                        ...
                                        <span>Time <strong><?php echo esc_html(Elev8_OS_Production_Catalog_Service::format_duration(Elev8_OS_Production_Catalog_Service::legacy_duration_to_seconds((float)($r['estimated_minutes']??0)))); ?></strong></span>
                                    </div>
    ...
    public static function ajax_quick_add_material(): void {
        self::guard();
        check_ajax_referer('elev8_quick_add_production_material', 'nonce');
        $result = Elev8_OS_Production_Catalog_Service::save_material(wp_unslash($_POST));
        if (is_wp_error($result)) {
            wp_send_json_error(['message' => $result->get_error_message()], 400);
        }
        $material = Elev8_OS_Production_Catalog_Service::material((int) $result);
        if (!$material) {
            wp_send_json_error(['message' => 'The material was saved but could not be loaded.'], 500);
        }
        wp_send_json_success([
            'material' => [
                'id' => (int) $material['id'],
                'name' => (string) $material['material_name'],
                'unit' => (string) $material['unit'],
                'unit_cost' => (float) $material['unit_cost'],
                'label' => sprintf('%s — $%s/%s', $material['material_name'], number_format_i18n((float) $material['unit_cost'], 4), $material['unit']),
            ],
        ]);
    }
