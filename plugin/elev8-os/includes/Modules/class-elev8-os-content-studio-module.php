...
final class Elev8_OS_Content_Studio_Module {
    ...
    private static function render_library(int $owner_user_id, bool $admin): string {
        ...
        ob_start();
        if (!$admin) {
            echo '<div class="elev8-portal-layout">'; Elev8_OS_Artist_Portal_Module::render_navigation('content_studio'); echo '<main class="elev8-content-main">';
        }
        ?>
    ...
    private static function campaign_wizard(int $owner_user_id, bool $admin, array $templates): void {
        ...
        <section class="elev8-campaign-wizard">
            ...
            <?php else:
                ...
                $preview=Elev8_OS_Template_Renderer::email_html($values,self::preview_variables($owner_user_id)); ?>
    ...
    private static function brand_form(): void { $b=Elev8_OS_Brand_Service::get(); ?>
        <section class="elev8-brand-manager"><div class="elev8-section-heading"><div><p class="elev8-eyebrow"><?php esc_html_e('Master Brand Template','elev8-os'); ?></p><h2><?php esc_html_e('Automatic Elev8 Branding','elev8-os'); ?></h2><p><?php esc_html_e('These settings power the universal email layout and future Content Studio channels.','elev8-os'); ?></p></div></div><form class="elev8-content-form" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><input type="hidden" name="action" value="elev8_os_content_save_brand"><?php wp_nonce_field('elev8_os_content_save_brand'); ?><div class="elev8-form-grid"><label><span><?php esc_html_e('Brand name','elev8-os'); ?></span><input type="text" name="brand_name" value="<?php echo esc_attr((string)$b['brand_name']); ?>"></label><label><span><?php esc_html_e('Website URL','elev8-os'); ?></span><input type="url" name="website_url" value="<?php echo esc_attr((string)$b['website_url']); ?>"></label><label><span><?php esc_html_e('Default button text','elev8-os'); ?></span><input type="text" name="default_cta_label" value="<?php echo esc_attr((string)$b['default_cta_label']); ?>"></label></div><div class="elev8-form-grid"><label><span><?php esc_html_e('Primary color','elev8-os'); ?></span><input type="color" name="primary_color" value="<?php echo esc_attr((string)$b['primary_color']); ?>"></label><label><span><?php esc_html_e('Header color','elev8-os'); ?></span><input type="color" name="secondary_color" value="<?php echo esc_attr((string)$b['secondary_color']); ?>"></label><label><span><?php esc_html_e('Email background','elev8-os'); ?></span><input type="color" name="background_color" value="<?php echo esc_attr((string)$b['background_color']); ?>"></label></div><label><span><?php esc_html_e('Footer text','elev8-os'); ?></span><input type="text" name="footer_text" value="<?php echo esc_attr((string)$b['footer_text']); ?>"></label><label><span><?php esc_html_e('Logo attachment ID','elev8-os'); ?></span><input type="number" min="0" name="logo_id" value="<?php echo esc_attr((string)$b['logo_id']); ?>"><small><?php esc_html_e('Use the WordPress Media Library attachment ID. A visual asset picker is planned for the next polish phase.','elev8-os'); ?></small></label><button class="button button-primary" type="submit"><?php esc_html_e('Save Brand Settings','elev8-os'); ?></button></form></section><?php
    }
