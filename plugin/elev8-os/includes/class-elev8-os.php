...
final class Elev8_OS {
    ...
    public static function enqueue_assets(string $hook): void {
        if (!in_array($hook, ['toplevel_page_elev8-os', 'elev8-os_page_elev8-artist-portal', 'elev8-os_page_elev8-development', 'elev8-os_page_elev8-system-status'], true)) {
            return;
        }
    ...
    private static function public_profile_content(int $employee_id): string {
        ...
        <div class="elev8-os elev8-public-profile">
          ...
          <?php if($store_assets):?><section class="elev8-panel elev8-artist-store"><div class="elev8-store-heading"><div><p class="elev8-store-eyebrow">Available at Elev8 Arts</p><h2>Artwork &amp; Products</h2></div><p>These items are managed directly by the artist through Elev8 OS.</p></div><div class="elev8-store-grid"><?php foreach($store_assets as $asset): $image=wp_get_attachment_image_url((int)$asset['image_attachment_id'],'large'); $is_sold=((string)$asset['status']==='sold');?><article class="elev8-store-card<?php echo $is_sold?' is-sold':'';?>"><div class="elev8-store-image"><?php if($image):?><img src="<?php echo esc_url($image);?>" alt="<?php echo esc_attr((string)$asset['title']);?>"><?php else:?><div class="elev8-store-image-missing">Image unavailable</div><?php endif;?><?php if($is_sold):?><span class="elev8-store-badge">Sold</span><?php endif;?></div><div class="elev8-store-body"><div class="elev8-store-title-row"><h3><?php echo esc_html((string)$asset['title']);?></h3><strong><?php echo $asset['price']===null?'Price unavailable':esc_html('$'.number_format_i18n((float)$asset['price'],2));?></strong></div><?php $meta=array_filter([(string)$asset['medium'],(string)$asset['dimensions']]); if($meta):?><p class="elev8-store-meta"><?php echo esc_html(implode(' · ',$meta));?></p><?php endif;?><?php if((string)$asset['description']!==''):?><p><?php echo esc_html(wp_trim_words((string)$asset['description'],32));?></p><?php endif;?><?php if(!$is_sold && $contacts): $contact=$contacts[0];?><p><a class="button" href="<?php echo esc_url($contact['url'],['http','https','mailto','tel']);?>"<?php echo preg_match('/^https?:/i',$contact['url'])?' target="_blank" rel="noopener"':'';?>>Ask about this item</a></p><?php endif;?></div></article><?php endforeach;?></div></section><?php endif;?>
