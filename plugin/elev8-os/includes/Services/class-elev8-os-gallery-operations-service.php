...
/** Canonical gallery location, placement, rack, and movement history service. */
final class Elev8_OS_Gallery_Operations_Service {
    ...
    public static function save_zone(array $data) {
        ...
        $row=['name'=>$name,'slug'=>$slug,'zone_type'=>sanitize_key((string)($data['zone_type']??'wall')),'capacity'=>absint($data['capacity']??0),'sort_order'=>absint($data['sort_order']??0),'active'=>empty($data['active'])?0:1,'notes'=>sanitize_textarea_field((string)($data['notes']??'')),'rack_number'=>absint($data['rack_number']??0),'board_number'=>absint($data['board_number']??0),'side_label'=>strtoupper(substr(sanitize_text_field((string)($data['side_label']??'')),0,1)),'assigned_artist_user_id'=>absint($data['assigned_artist_user_id']??0),'board_status'=>$status,'updated_at'=>$now];
    ...
    public static function create_artist_portal_rack(int $rack_number,int $board_count,int $capacity_per_side=1) {
        if($rack_number<1 || $board_count<1) return new WP_Error('rack','Rack number and board count are required.');
        $board_count=min(100,$board_count); $capacity_per_side=max(1,min(20,$capacity_per_side)); $created=0;
        foreach(range(1,$board_count) as $board){ foreach(['A','B'] as $side){
            $name=sprintf('Artist Portal Rack %d — Art Board %d — Side %s',$rack_number,$board,$side);
            $result=self::save_zone(['name'=>$name,'slug'=>sprintf('artist-portal-rack-%d-board-%d-side-%s',$rack_number,$board,strtolower($side)),'zone_type'=>'artist_portal_rack','capacity'=>$capacity_per_side,'sort_order'=>9000+($rack_number*1000)+($board*2)+($side==='B'?1:0),'active'=>1,'rack_number'=>$rack_number,'board_number'=>$board,'side_label'=>$side,'board_status'=>'available']);
            if(is_wp_error($result)){ if($result->get_error_code()==='zone_save') continue; return $result; } $created++;
        }} return $created;
    }
    ...
    public static function get_placement(int $asset_id): ?array { global $wpdb; $r=$wpdb->get_row($wpdb->prepare('SELECT p.*,z.name zone_name,z.zone_type,z.rack_number,z.board_number,z.side_label FROM '.self::placements_table().' p LEFT JOIN '.self::zones_table().' z ON z.id=p.zone_id WHERE p.asset_id=%d',$asset_id),ARRAY_A); return is_array($r)?$r:null; }
    public static function place_asset(int $asset_id, int $zone_id, string $position='', string $note='') {
        ...
        if(in_array((string)$zone['board_status'],['maintenance'],true)) return new WP_Error('zone_status','That board side is in maintenance.');
        $assigned=absint($zone['assigned_artist_user_id']??0); if($assigned && $assigned!==absint($asset['owner_user_id']??0)) return new WP_Error('artist_assignment','That board side is reserved for another artist.');
        ...
        self::record($asset_id,$old?'moved':'displayed',absint($old['zone_id']??0),$zone_id,$note); return true;
    }
    ...
    public static function get_artists(): array { global $wpdb; $a=Elev8_OS_Asset_Service::table_name(); $r=$wpdb->get_results("SELECT DISTINCT u.ID,u.display_name,u.user_email FROM {$a} a JOIN {$wpdb->users} u ON u.ID=a.owner_user_id ORDER BY u.display_name",ARRAY_A); return is_array($r)?$r:[]; }
    public static function dashboard(int $owner_user_id=0): array {
        global $wpdb; $a=Elev8_OS_Asset_Service::table_name(); $p=self::placements_table(); $z=self::zones_table(); $owner_user_id=absint($owner_user_id); $owner_where=$owner_user_id?$wpdb->prepare(' AND a.owner_user_id=%d',$owner_user_id):'';
        $summary=$wpdb->get_row("SELECT COUNT(*) total, SUM(status='available') available, SUM(status='reserved') reserved, SUM(status='sold') sold, SUM(status='available' AND location='at_elev8') on_site, COALESCE(SUM(CASE WHEN status='available' THEN price*quantity ELSE 0 END),0) available_value FROM {$a}".($owner_user_id?$wpdb->prepare(' WHERE owner_user_id=%d',$owner_user_id):''),ARRAY_A)?:[];
        ...
        $unplaced=$wpdb->get_results("SELECT a.*,u.display_name artist_name FROM {$a} a LEFT JOIN {$p} p ON p.asset_id=a.id AND p.placement_status='displayed' LEFT JOIN {$wpdb->users} u ON u.ID=a.owner_user_id WHERE a.status IN ('available','reserved') AND a.location='at_elev8' AND p.id IS NULL {$owner_where} ORDER BY u.display_name,a.title LIMIT 300",ARRAY_A);
        $placed=$wpdb->get_results("SELECT a.*,p.zone_id,p.position_label,p.placed_at,z.name zone_name,z.zone_type,z.rack_number,z.board_number,z.side_label,u.display_name artist_name FROM {$p} p JOIN {$a} a ON a.id=p.asset_id LEFT JOIN {$z} z ON z.id=p.zone_id LEFT JOIN {$wpdb->users} u ON u.ID=a.owner_user_id WHERE p.placement_status='displayed' {$owner_where} ORDER BY u.display_name,z.sort_order,a.title",ARRAY_A);
        return ['summary'=>$summary,'zones'=>is_array($zones)?$zones:[],'unplaced'=>is_array($unplaced)?$unplaced:[],'placed'=>is_array($placed)?$placed:[],'artists'=>self::get_artists(),'artist_filter'=>$owner_user_id];
    }
    public static function owner_snapshot(int $owner): array {
        ...
        $rows=$wpdb->get_results($wpdb->prepare("SELECT a.id,a.title,a.status,a.price,p.placement_status,p.placed_at,z.name zone_name,z.zone_type,z.rack_number,z.board_number,z.side_label,DATEDIFF(CURDATE(),p.placed_at) days_displayed FROM {$a} a LEFT JOIN {$p} p ON p.asset_id=a.id LEFT JOIN {$z} z ON z.id=p.zone_id WHERE a.owner_user_id=%d ORDER BY a.updated_at DESC",$owner),ARRAY_A);
