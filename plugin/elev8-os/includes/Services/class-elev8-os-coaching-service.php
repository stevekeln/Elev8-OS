...
final class Elev8_OS_Coaching_Service {
    ...
    private static function event_host_recommendations(WP_User $user): array {
        ...
        if (empty($profile['published'])) {
            $items[] = self::item('host.profile', __('Prepare your public event-host profile', 'elev8-os'), __('Guests cannot currently view a published host profile for you.', 'elev8-os'), 'medium', 100, __('The Event Host Dashboard service reports that no published public host profile is connected to your account.', 'elev8-os'), '', __('Profile Editor Coming Soon', 'elev8-os'));
        }
        if (!$items) {
